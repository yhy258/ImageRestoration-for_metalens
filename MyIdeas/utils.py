...
from light_NAFNet import Light_NAFNet
import pyiqa

def define_models(config, device):
    if config.model_type == "NAFNet":
    ...
    if config.mode == "Prior_NAFNet" :
        vae = VanillaVAE(in_channels=config.img_channel, latent_dim=config.latent_dim, head_nums=config.head_num)
        return net, vae
...
"""
def save_models(save_path, mode, model_type, model, optimizer, scheduler, loss_info, epoch):
    if isinstance(model, list):
        ...
        torch.save({
            ...
            "loss_info": loss_info
        }, os.path.join(save_path, f"{mode}_{epoch}.pt"))
    else :
        NAFNet = model
        torch.save({
            "model": NAFNet.state_dict(),
            "loss_info": loss_info
        }, os.path.join(save_path, f"{mode}_{model_type}_{epoch}.pt"))
